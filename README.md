...
Bloque de detección de línea: * Se encarga de procesar las señales del sensor TCRT5000 y decidir la dirección del vehículo.
Bloque de control de motores: * Utiliza las señales generadas por el bloque de detección de línea para activar los motores a través del driver L298N.
...
Si el sensor izquierdo detecta la línea pero el derecho no, el vehículo gira hacia la derecha.
Si ninguno de los sensores detecta la línea, el vehículo se detiene para evitar que salga del recorrido.
...
Motores y driver: Los motores estan conectados a el driver L298N. Y este a su vez esta conectado a una fiente DC variable dado que requieren un voltaje mayor para su operación. La utilización de un módulo powerbank asegura que la alimentación sea continua y que la FPGA tenga protección contra picos de voltaje o caídas inesperadas de corriente.
...
Montaje del Hardware: 1.1. Sensores TCRT5000: El primer paso fue la instalación de los sensores TCRT5000, responsables de detectar la línea. Estos sensores se montaron en la parte inferior del chasis del vehículo para que pudieran detectar la línea en el suelo.
...
Desarrollo del firmware en la FPGA: 2.1. Desarrollo de la lógica de detección de línea: La lógica de detección de línea fue implementada utilizando compuertas lógicas en la FPGA. Para ello, se escribió el código en Verilog, que define la interacción entre las señales de entrada de los sensores y las señales de salida hacia los motores.
...
Si ningún sensor detecta la línea (0 y 0), ambos motores se detienen. 2.2. Generación de PWM para control de motores: El control de velocidad de los motores fue implementado utilizando señales PWM generadas directamente en la FPGA.
